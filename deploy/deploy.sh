...
#   api   : sube la app Laravel a ~/domains/api.petru.com.ar/laravel (sin .env,
#           sin storage/ ni vendor/), el puente a public_html, corre composer,
#           migraciones, storage:link y cachea config/rutas.
...
#   panel : npm run build y sube dist a ~/domains/panel.petru.com.ar/public_html.
# =============================================================================
set -euo pipefail
...
REMOTO_API="domains/api.$DOMINIO"
REMOTO_SITIO="domains/$DOMINIO/public_html"
REMOTO_PANEL="domains/panel.$DOMINIO/public_html"
...
deploy_env() {
  ...
  $SSH "mkdir -p ~/$REMOTO_API/laravel; if [ -f ~/$REMOTO_API/laravel/.env ]; then echo 'Ya hay un .env en el servidor: no se pisa.'; cat >/dev/null; else cat > ~/$REMOTO_API/laravel/.env && echo '.env creado'; fi" < "$archivo"
}

deploy_api() {
  echo "== API =="
  subir "$RAIZ/API" "$REMOTO_API/laravel" \
  ...
  subir "$RAIZ/deploy/api-public_html" "$REMOTO_API/public_html"

  $SSH bash -s <<EOF
set -e
cd ~/$REMOTO_API/laravel
if [ ! -f .env ]; then
  echo "!! Falta ~/$REMOTO_API/laravel/.env — corré primero: ./deploy/deploy.sh env"
  exit 1
fi
...
# El link de storage apunta a public_html/storage (el puente usa usePublicPath).
rm -rf ../public_html/storage
ln -s ~/$REMOTO_API/laravel/storage/app/public ../public_html/storage
