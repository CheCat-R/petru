...
/**
 * ⭐ Lo que el taller ve: tres módulos reales, conectados a Laravel.
 * El árbol del ERP (más abajo) se suma solo con VITE_MOSTRAR_ERP=true.
 */
const SECCIONES_PETRU = [
  {
    title: "PËTRU",
    items: [
      ...
      // `badge`: clave de useNovedades() con el contador que se muestra al lado.
      { text: "Pedidos", icon: <ShoppingBagOutlinedIcon fontSize="small" />, path: "/pedidos", badge: "ordersToPrepare" },
      { text: "Consultas", icon: <ForumOutlinedIcon fontSize="small" />, path: "/consultas", badge: "newInquiries" },
      { text: "Estadísticas", icon: <InsightsOutlinedIcon fontSize="small" />, path: "/estadisticas" },
      { text: "Sitio web", icon: <LanguageOutlinedIcon fontSize="small" />, path: "/sitio" },
      {
        text: "Integraciones",
        icon: <ExtensionOutlinedIcon fontSize="small" />,
        path: "/integraciones",
        children: [
          { text: "MercadoPago", path: "/integraciones/mercadopago" },
          { text: "Envíos", path: "/integraciones/envios" },
        ],
      },
    ],
  },
  {
    title: "CUENTA",
    items: [
      { text: "Mi perfil", icon: <PersonOutlineOutlinedIcon fontSize="small" />, path: "/perfil" },
      { text: "Usuarios", icon: <GroupOutlinedIcon fontSize="small" />, path: "/usuarios" },
    ],
  },
];
...
// Con el ERP visible, Dashboard/Productos/Pedidos del ERP quedan tapados por los
// de Pëtru (mismo path): se sacan del menú para no ofrecer dos entradas iguales.
const sinDuplicados = (secciones) =>
  secciones
    .map((sec) => ({
      ...sec,
      items: sec.items.filter((it) => !["/", "/productos", "/pedidos"].includes(it.path)),
    }))
    .filter((sec) => sec.items.length > 0);

export const NAV_SECTIONS = MOSTRAR_ERP
  ? [...SECCIONES_PETRU, ...sinDuplicados(SECCIONES_ERP)]
  : SECCIONES_PETRU;
