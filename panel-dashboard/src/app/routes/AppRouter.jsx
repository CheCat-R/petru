...
/**
 * ⭐ Pëtru arranca con tres módulos reales conectados a Laravel: Catálogo,
 * Pedidos y Consultas. El resto del ERP sigue en el repo, pero fuera del
 * router y del menú hasta que VITE_MOSTRAR_ERP=true. Cuando se activa, las
 * rutas de Productos y Pedidos del ERP quedan tapadas por las de Pëtru (mismo
 * path, y las reales van primero): ese par de pantallas ya tiene reemplazo.
 */
const MOSTRAR_ERP = import.meta.env.VITE_MOSTRAR_ERP === "true";

const PetruDashboard = lazy(() => import("../../modules/petru/Dashboard"));
const PetruProductos = lazy(() => import("../../modules/petru/catalogo/Productos"));
const PetruProductoForm = lazy(() => import("../../modules/petru/catalogo/ProductoForm"));
const PetruCategorias = lazy(() => import("../../modules/petru/catalogo/Categorias"));
const PetruPedidos = lazy(() => import("../../modules/petru/pedidos/Pedidos"));
const PetruPedidoDetalle = lazy(() => import("../../modules/petru/pedidos/PedidoDetalle"));
const PetruConsultas = lazy(() => import("../../modules/petru/consultas/Consultas"));
const PetruSitio = lazy(() => import("../../modules/petru/sitio/Sitio"));
const PetruEstadisticas = lazy(() => import("../../modules/petru/estadisticas/Estadisticas"));
const PetruMercadoPago = lazy(() => import("../../modules/petru/integraciones/MercadoPago"));
const PetruEnvios = lazy(() => import("../../modules/petru/integraciones/Envios"));
const PetruPerfil = lazy(() => import("../../modules/petru/cuenta/Perfil"));
const PetruUsuarios = lazy(() => import("../../modules/petru/cuenta/Usuarios"));
const OlvidePassword = lazy(() => import("../../modules/auth/OlvidePassword"));
const RestablecerPassword = lazy(() => import("../../modules/auth/RestablecerPassword"));

// Componente para proteger las rutas privadas
const PrivateRoute = () => {
  const { isAuthenticated, cargando } = useAuth();
  // Hasta que la API conteste si hay cookie de sesión, no se redirige a ningún lado:
  // mandar al login y volver medio segundo después parece un bug.
  ...
  return (
    <BrowserRouter>
      ...
      <Suspense fallback={<div className="route-loading" aria-busy="true" />}>
        ...
        <Route path="/olvide" element={<PublicRoute><OlvidePassword /></PublicRoute>} />
        <Route path="/restablecer" element={<PublicRoute><RestablecerPassword /></PublicRoute>} />

        {/* Rutas Privadas Protegidas con Layout */}
        <Route path="/" element={<PrivateRoute />}>
          {/* ── Pëtru: módulos reales ─────────────────────────────────── */}
          <Route index element={<PetruDashboard />} />
          <Route path="productos">
            <Route index element={<PetruProductos />} />
            <Route path="nuevo" element={<PetruProductoForm />} />
            <Route path="categorias" element={<PetruCategorias />} />
            <Route path=":id" element={<PetruProductoForm />} />
          </Route>
          <Route path="pedidos">
            <Route index element={<PetruPedidos />} />
            <Route path=":numero" element={<PetruPedidoDetalle />} />
          </Route>
          <Route path="consultas" element={<PetruConsultas />} />
          <Route path="sitio" element={<PetruSitio />} />
          <Route path="estadisticas" element={<PetruEstadisticas />} />
          <Route path="integraciones" element={<Navigate to="/integraciones/mercadopago" replace />} />
          <Route path="integraciones/mercadopago" element={<PetruMercadoPago />} />
          <Route path="integraciones/envios" element={<PetruEnvios />} />
          <Route path="perfil" element={<PetruPerfil />} />
          <Route path="usuarios" element={<PetruUsuarios />} />

          {/* ── ERP CheCAT: oculto hasta VITE_MOSTRAR_ERP=true ───────────── */}
          {MOSTRAR_ERP && (
            <>
